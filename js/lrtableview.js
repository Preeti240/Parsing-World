...
function formatLRTable(lrTable) {
	var result = "<h3 align='center'><b>With the above information, we generate the LR Sintactic Table of this grammar:</b></div><table border=\"1\" class='table table-bordered'>";
	var grammar = lrTable.grammar;
	result += "<thead>";
	result += "<tr><th colspan=\"" + (1 + grammar.terminals.length + 1 + grammar.nonterminals.length) + "\">LR Sintactic Table</th></tr><tr><th rowspan=\"2\"> State </th><th rowspan=\"1\" colspan=\"" + (grammar.terminals.length + 1) + "\"> Action </th><th colspan=\"" + grammar.nonterminals.length + "\"> Deviation (goto) </th></tr>\n<tr>";
...
function formatActionElement(actionElement) {
	return actionElement.toString()
			.replace("r0", "<span style=\"color: black;\">Accepts</span>")
